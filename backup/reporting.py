...
def generate_performance_report(days: int = DEFAULT_REPORT_DAYS) -> pd.DataFrame:
    ...
    avg_win = (
        trades_df[trades_df["profit"] > 0]["profit"].mean() if winning_trades > 0 else 0
    )
    avg_loss = (
        trades_df[trades_df["profit"] <= 0]["profit"].mean() if losing_trades > 0 else 0
    )
...
def create_performance_charts(days: int = DEFAULT_REPORT_DAYS) -> bool:
    ...
    try:
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join("reports", "charts", "profit_distribution.png"), dpi=CHART_DPI
        )
        plt.close()
        ...
        plt.figure(figsize=CHART_SIZE)
        cumulative_profit = (
            trades_df.sort_values("exit_date").set_index("exit_date")["profit"].cumsum()
        )
        ...
        plt.grid(True, alpha=0.3)
        plt.fill_between(
            cumulative_profit.index,
            cumulative_profit.values,
            where=(cumulative_profit.values > 0),
            alpha=0.2,
            color="green",
        )
        plt.fill_between(
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join("reports", "charts", "cumulative_profit.png"), dpi=CHART_DPI
        )
        plt.close()
        ...
        plt.figure(figsize=CHART_SIZE)
        signal_win_rates = (
            trades_df.groupby("type")["profit"]
            .apply(lambda x: (x > 0).mean() if len(x) > 0 else 0)
            .reset_index()
        )
        ...
        if not signal_win_rates.empty:
            ...
            plt.tight_layout()
            plt.savefig(
                os.path.join("reports", "charts", "win_rate_by_signal.png"),
                dpi=CHART_DPI,
            )
            plt.close()
        ...
        plt.figure(figsize=CHART_SIZE)
        top_symbols = (
            trades_df.groupby("symbol")["profit"].sum().nlargest(10).reset_index()
        )
        ...
        plt.figure(figsize=CHART_SIZE)
        exit_performance = (
            trades_df.groupby("exit_reason")["profit"].mean().reset_index()
        )
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join("reports", "charts", "exit_reason_performance.png"),
            dpi=CHART_DPI,
        )
        plt.close()
...
def current_positions_report() -> pd.DataFrame:
    ...
    try:
        if not pos_df.empty:
            ...
            plt.tight_layout()
            plt.savefig(
                os.path.join("reports", "charts", "current_positions.png"),
                dpi=CHART_DPI,
            )
            plt.close()
            ...
            plt.tight_layout()
            plt.savefig(
                os.path.join("reports", "charts", "position_distribution.png"),
                dpi=CHART_DPI,
            )
            plt.close()
...
def export_report_to_html(days: int = DEFAULT_REPORT_DAYS) -> bool:
    ...
    try:
        ...
        report_path = os.path.join(
            "reports", f"performance_report_{datetime.now().strftime('%Y%m%d')}.html"
        )
...
def main(days: int = DEFAULT_REPORT_DAYS, export_html: bool = True):
    ...
    print("\n" + "=" * 80)
    print(
        f" nGS TRADING SYSTEM PERFORMANCE REPORT - {datetime.now().strftime('%Y-%m-%d')}"
    )
    print("=" * 80)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Generate nGS Trading System performance reports"
    )
    parser.add_argument(
        "--days",
        type=int,
    ...
    parser.add_argument(
        "--no-html", action="store_true", help="Skip HTML report export"
    )
