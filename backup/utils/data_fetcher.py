...
class DataFetcher:
    def __init__(self, api_key=None):
        ...
        self.api_key = (
            api_key or POLYGON_API_KEY
        )  # Use config API key instead of environment variable
        if not self.api_key:
    ...
    def _load_last_fetch_dates(self):
        ...
        return {
            "daily": {"full_history_date": None, "last_update": None},
            "minute": {"full_history_date": None, "last_update": None},
        }
    ...
    def fetch_daily_data(self, force_full_history=False):
            ...
            try:
                ...
                if not symbol_data.empty:
                    ...
                    if os.path.exists(existing_file) and not force_full_history:
                        ...
                        symbol_data.to_csv(existing_file, index=False)
                        logger.info(
                            f"Saved daily data for {symbol} with {len(symbol_data)} bars"
                        )
        ...
        logger.info(
            f"Completed daily data fetch. Retrieved data for {len(all_data)} symbols"
        )
        return all_data
    ...
    def fetch_minute_data(self, force_full_history=False):
        ...
        # Check if we've already fetched full history
        if (
            self.last_fetch_dates["minute"]["full_history_date"]
            and not force_full_history
        ):
            # We've already fetched full history, just get updates
                ...
                if not symbol_data.empty:
                    ...
                    if os.path.exists(existing_file) and not force_full_history:
                        try:
                            ...
                            combined_data.to_csv(existing_file, index=False)
                            logger.info(
                                f"Updated minute data for {symbol} with {len(symbol_data)} new bars"
                            )
                        except Exception as e:
                            logger.error(f"Error updating data for {symbol}: {e}")
                            # If update fails, save the new data
                        ...
                        symbol_data.to_csv(existing_file, index=False)
                        logger.info(
                            f"Saved minute data for {symbol} with {len(symbol_data)} bars"
                        )
        ...
        logger.info(
            f"Completed minute data fetch. Retrieved data for {len(all_data)} symbols"
        )
        return all_data
    ...
    def save_trade(self, trade_data):
        ...
        logger.info(
            f"Trade recorded for {trade_data.get('symbol')} - {trade_data.get('side')}"
        )
