...
from data_manager import (
    ...
    save_signals,
    get_positions,
    initialize as init_data_manager,
    RETENTION_DAYS,
...
class NGSStrategy:
    ...
    def __init__(self, account_size: float = 1000000, data_dir: str = "data"):
        ...
        self.min_positions_for_scaling_up = (
            5  #  Minimum positions required for upward scaling
        )
        ...
        print(
            f" L/S Ratio Adjust:     {'ENABLED' if self.ls_ratio_enabled else 'DISABLED'}"
        )
        print(
            f" Sector Management:    {'ENABLED' if self.sector_allocation_enabled else 'DISABLED'}"
        )
        print(f"{'='*70}")

        logger.info(
            f"nGS Strategy initialized with {self.retention_days}-day data retention"
        )
        ...
        logger.info(
            f" Min positions for scale-up: {self.min_positions_for_scaling_up}"
        )
        logger.info(
    ...
    def check_me_rebalancing_needed(self) -> Optional[str]:
        ...
        try:
            ...
            if current_me < self.me_target_min:
                ...
                print(
                    f" SCALE DOWN NEEDED: {current_me:.1f}% > {self.me_target_max}%"
                )
                logger.info(
    ...
    def perform_me_rebalancing(self, action: str) -> bool:
        ...
        try:
            ...
            if action == "scale_up":
                ...
                target_scaling = (
                    self.me_target_min / current_me if current_me > 0 else 1.5
                )
                target_scaling = min(target_scaling, 2.0)  # Cap at 2x for safety
            elif action == "scale_down":
                # Scale down to reach maximum M/E target
                target_scaling = (
                    self.me_target_max / current_me if current_me > 0 else 0.8
                )
                target_scaling = max(
                    target_scaling, 0.5
                )  # Minimum 50% scaling for safety
            else:
            ...
            logger.info(
                f"M/E Rebalancing: {action} with {target_scaling:.2f}x scaling factor"
            )
            logger.info(
    ...
    def calculate_historical_me_ratio(
        ...
        return self.me_calculator.calculate_daily_me_ratio()[
            "ME_Ratio"
        ]  # Historical is now daily snapshot
    ...
    def record_historical_me_ratio(
        ...
        self.me_calculator.calculate_daily_me_ratio(
            date_str
        )  # Automatically appends to history
    ...
    def _process_exit(
        self, df: pd.DataFrame, i: int, symbol: str, position: Dict
    ) -> None:
    ...
    def run(self, data: Dict[str, pd.DataFrame]) -> Dict[str, pd.DataFrame]:
        ...
        if self.cash > 0:
            ...
            print(
                f"Calculated M/E:         {calculated_me:.2f}% (should match Final M/E)"
            )
    ...
    def _display_sector_summary(self, sector_report: Dict):
        ...
        if current_exposure:
            ...
            for sector, data in current_exposure.items():
                ...
                print(
                    f"  {sector:22s}: {current_weight:5.1f}% (target: {target_weight:5.1f}%) - {data['count']} positions"
                )
        ...
        if rebalance_needs:
            ...
            if needs_rebalancing:
                ...
                for sector in needs_rebalancing:
                    ...
                    print(
                        f"  {sector:22s}: {data['action']:4s} {data['difference']:+5.1%} "
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            f"   Status:              {'ACTIVE' if strategy.me_rebalancing_enabled else 'INACTIVE'}"
        )
        print(
            f"   Target Range:        {strategy.me_target_min:.1f}% - {strategy.me_target_max:.1f}%"
        )
        print(f"   Min Positions:       {strategy.min_positions_for_scaling_up}")
        print(
            f"   Sector Management:   {'ENABLED' if strategy.sector_allocation_enabled else 'DISABLED'}"
        )
        ...
        print(
            f"Data period:          {strategy.cutoff_date.strftime('%Y-%m-%d')} to {datetime.now().strftime('%Y-%m-%d')}"
        )
        print(
            f"M/E rebalancing:      {'ENABLED' if strategy.me_rebalancing_enabled else 'DISABLED'}"
        )
        print(
            f"Sector management:    {'ENABLED' if strategy.sector_allocation_enabled else 'DISABLED'}"
        )
        ...
        if strategy.trades:
            ...
            sorted_symbols = sorted(
                symbol_profits.items(), key=lambda x: x[1], reverse=True
            )
        ...
        if strategy.trades:
            ...
            for trade in strategy.trades[-10:]:
                ...
                print(
                    f"  {trade['symbol']} ({sector:15s}) {trade['type']:5s} | "
        ...
        if total_positions > 0:
            ...
            for pos in all_pos[:10]:
                ...
                print(
                    f"  {side} {pos['symbol']:6s} ({sector:15s}): {shares:4d} shares @ ${pos['entry_price']:7.2f}"
                )
        ...
        print(
            f" M/E rebalancing: {'ACTIVE' if strategy.me_rebalancing_enabled else 'INACTIVE'}"
        )
        print(
            f" Sector management: {'ENABLED' if strategy.sector_allocation_enabled else 'DISABLED'}"
        )
