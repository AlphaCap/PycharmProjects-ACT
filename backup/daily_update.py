...
if not POLYGON_API_KEY:
    ...
    logger.warning(
        "No Polygon API key found. Please set POLYGON_API_KEY environment variable."
    )
...
def download_data_parallel(symbols: List[str]) -> Dict[str, pd.DataFrame]:
    ...
    with ThreadPoolExecutor(max_workers=CONFIG["max_workers"]) as executor:
        future_to_symbol = {
            executor.submit(
                get_polygon_daily_data, symbol, CONFIG["history_days"]
            ): symbol
                ...
                if response.status_code == 200:
                    ...
                    if "results" in data:
                        ...
                        for ticker in data["results"]:
                            ...
                            if (
                                ticker.get("type") == "CS"  # Common Stock
                                and ticker.get("market") == "stocks"
                                and ticker.get("primary_exchange") in ["XNYS", "XNAS"]
                            ):
                        ...
                        if len(symbols) >= 450:
                            logger.info(
                                f"Retrieved {len(symbols)} potential SP500 symbols from Polygon"
                            )
