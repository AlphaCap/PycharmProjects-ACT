...
with tab2:
    ...
    try:
        ...
        if os.path.exists(csv_path):
            ...
            long_df = (
                df[df["side"].str.lower().str.startswith("b")].copy()
...
with tab3:
    ...
    try:
        ...
        if os.path.exists(csv_path):
            ...
            short_df = (
                df[df["side"].str.lower().str.startswith("s")].copy()
                if "side" in df.columns
                else pd.DataFrame()
            )
...
with col1:
    ...
    for signal in signals:
        st.markdown(
            f"- **{signal['date']}** | **{signal['symbol']}** | **{signal['signal']}** | {signal['strategy']} at `{signal['price']}`"
        )
...
with col2:
    ...
    system_status = [
        {
            "timestamp": "07/04/25 16:30",
            "system": "nGS",
            "message": "Daily signals generated successfully",
        },
        {
            "timestamp": "07/04/25 16:15",
            "system": "Intraday",
            "message": "Trading session closed, 8 signals processed",
        },
        {
            "timestamp": "07/04/25 15:45",
            "system": "Daily",
            "message": "Portfolio rebalanced, 2 new positions",
        },
        {
            "timestamp": "07/04/25 09:30",
            "system": "Intraday",
            "message": "Trading session started",
        },
        {
            "timestamp": "07/03/25 16:30",
            "system": "nGS",
            "message": "Daily signals generated successfully",
        },
    ]
    ...
    for status in system_status:
        st.markdown(
            f"- **{status['timestamp']}** | **{status['system']}** | {status['message']}"
        )

# Footer with additional info
st.markdown("---")
st.caption(
    "Alpha Trading Systems Dashboard - For additional support, please contact the trading desk."
)
