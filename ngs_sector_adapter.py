...
try:
    ...
    print("⚠️  PyBroker not available. Install with: pip install lib-pybroker")
...
try:
    ...
    print("⚠️  Data manager not found - using fallback data loading")
...
class NGSSectorAdapter:
    ...
    def __init__(self, config_dir: str = "optimization_framework/config"):
        ...
        print(f"📊 Optimized sectors available: {len(self.param_manager.get_all_optimized_sectors())}")
    ...
    def load_symbol_data(self, symbol: str, start_date: str = None, end_date: str = None) -> Optional[pd.DataFrame]:
        ...
        try:
            if DATA_MANAGER_AVAILABLE:
                ...
                print(f"⚠️  No data loading mechanism available for {symbol}")
                return None
    ...
    def prepare_pybroker_data(self, symbols: List[str], start_date: str = None, end_date: str = None) -> Optional[pd.DataFrame]:
        """
        ...
        if all_data:
            ...
            print(f"✅ Combined data: {len(combined_df)} total records for {len(symbols)} symbols")
            return combined_df
        else:
            print("❌ No data loaded for any symbols")
            return None
    
    def create_ngs_indicators(self, df: pd.DataFrame, symbol: str, params: Dict) -> pd.DataFrame:
        """
        ...
        df['ngs_long_signal'] = (
            ...
            (df['market_efficiency'] <= me_max) &  # Not too efficient (avoiding whipsaws)
            (df['close'] > params.get('MinPrice', 10)) &  # Price filters
            (df['close'] < params.get('MaxPrice', 500)) &
            (df['volume'] > df['volume'].rolling(20).mean() * 0.8)  # Volume confirmation
        )
    ...
    def ngs_entry_signal(self, ctx: ExecContext):
        ...
        if symbol not in self.symbol_parameters:
            self.symbol_parameters[symbol] = self.param_manager.get_parameters_for_symbol(symbol)
        ...
        entry_conditions = (
            bb_position <= 20 and  # Near lower Bollinger Band
            me_min <= market_efficiency <= me_max and  # Market efficiency in range
            min_price <= current_price <= max_price and  # Price filters
            not ctx.long_pos()  # Not already in position
        )
    ...
    def ngs_exit_signal(self, ctx: ExecContext):
        ...
        symbol = ctx.symbol
        params = self.symbol_parameters.get(symbol, self.param_manager.default_parameters)
        ...
        exit_conditions = (
            current_price >= profit_target or  # Profit target hit
            current_price <= stop_loss or      # Stop loss hit
            bars_held >= max_hold_days         # Time-based exit
        )
        
        if exit_conditions:
            ctx.sell_all()
    
    def create_pybroker_strategy(self, symbols: List[str], start_date: str = None, end_date: str = None) -> Optional[pb.Strategy]:
        """
        ...
        print(f"🏗️  Creating PyBroker strategy for {len(symbols)} symbols")
        ...
        market_efficiency = pb.indicator('market_efficiency', lambda data: data['market_efficiency'])
    ...
    def run_backtest(self, symbols: List[str], start_date: str = None, end_date: str = None, 
                    show_summary: bool = True) -> Optional[Dict]:
        """
        ...
        try:
            ...
            portfolio = result.portfolio if hasattr(result, 'portfolio') else pd.DataFrame()
    ...
    def _analyze_backtest_results(self, trades: pd.DataFrame, portfolio: pd.DataFrame, symbols: List[str]) -> Dict:
        ...
        if not trades.empty:
            ...
            analysis['win_rate'] = (analysis['winning_trades'] / analysis['total_trades'] * 100) if analysis['total_trades'] > 0 else 0
            ...
            if 'return_pct' in trades.columns:
                ...
                analysis['win_avg_return'] = trades[trades['pnl'] > 0]['return_pct'].mean() if analysis['winning_trades'] > 0 else 0
                analysis['loss_avg_return'] = trades[trades['pnl'] <= 0]['return_pct'].mean() if analysis['losing_trades'] > 0 else 0
            
            # By symbol analysis
            symbol_stats = {}
            for symbol in symbols:
                symbol_trades = trades[trades['symbol'] == symbol] if 'symbol' in trades.columns else pd.DataFrame()
        ...
        if not portfolio.empty and 'equity' in portfolio.columns:
            ...
            analysis['total_return_pct'] = ((final_equity - initial_equity) / initial_equity * 100) if initial_equity > 0 else 0
    ...
    def _print_backtest_summary(self, analysis: Dict):
        ...
        if 'by_symbol' in analysis and analysis['by_symbol']:
            ...
            for symbol, stats in analysis['by_symbol'].items():
                print(f"{symbol}: {stats['trades']} trades, {stats['win_rate']:.1f}% win rate, ${stats['total_pnl']:.2f} PnL")
    ...
    def update_sector_parameters(self, sector: str, new_parameters: Dict):
        ...
        print(f"✅ Updated {sector} parameters and refreshed {len(symbols_to_update)} symbol caches")
...
if __name__ == "__main__":
    ...
    try:
        data = adapter.prepare_pybroker_data(test_symbols, start_date='2023-01-01', end_date='2023-12-31')
        if data is not None:
    ...
    try:
        strategy = adapter.create_pybroker_strategy(test_symbols[:2], start_date='2023-01-01', end_date='2023-06-30')
        if strategy:
    ...
    print(f"   results = adapter.run_backtest(['AAPL', 'MSFT'], start_date='2023-01-01', end_date='2023-12-31')")
