...
class PerformanceObjective(ABC):
    ...
    @abstractmethod
    def calculate_fitness(self, trades: List[Dict], equity_curve: pd.Series, 
...
class LinearEquityObjective(PerformanceObjective):
    ...
    def calculate_fitness(self, trades: List[Dict], equity_curve: pd.Series, 
        ...
        try:
            ...
            fitness = (linearity_score * self.smoothness_weight + 
...
class MaxROIObjective(PerformanceObjective):
    ...
    def get_objective_description(self) -> str:
        return "Maximum ROI: Optimize for highest total returns, accept higher volatility"
...
class ObjectiveManager:
    ...
    def get_objective(self, name: str) -> PerformanceObjective:
        ...
        if name not in self.objectives:
            raise ValueError(f"Objective '{name}' not found. Available: {list(self.objectives.keys())}")
    ...
    def add_custom_objective(self, name: str, fitness_function: callable,
        ...
        self.objectives[name] = CustomObjective(name, fitness_function, 
                                              strategy_preferences, description)
        print(f"✅ Added custom objective: {name}")
    
    def compare_objectives(self, trades: List[Dict], equity_curve: pd.Series) -> pd.DataFrame:
