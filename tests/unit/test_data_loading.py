    ...
    for sector, etf in sector_etfs.items():
        ...
        if file_path.exists():
            ...
            assert pd.to_datetime(df['Date']).is_monotonic_increasing, \
                f"Dates not in chronological order in {etf} file"
...
def test_trading_data_directory():
    ...
    if stock_files:
        ...
        for file_path in stock_files[:sample_size]:
            ...
            for col in numeric_cols:
                ...
                assert (df[col] > 0).all(), \
                    f"Found non-positive values in {file_path.name} {col}"


def test_etf_data_sufficiency_for_walk_forward():
    ...
    for file_path in etf_dir.glob("*_historical.csv"):
        try:
            ...
            date_range = (df['Date'].max() - df['Date'].min()).days / 30.44  # Approximate months
    ...
    for symbol, info in results.items():
        ...
        print(f"{status} {symbol}: {info['months_of_data']:.1f} months "
...
def test_etf_data_consistency():
    ...
    for file_path in etf_dir.glob("*_historical.csv"):
        ...
        assert (df['High'] >= df['Low']).all(), f"{symbol}: High prices must be >= Low prices"
        assert (df['High'] >= df['Close']).all(), f"{symbol}: High prices must be >= Close prices"
        assert (df['Close'] >= df['Low']).all(), f"{symbol}: Close prices must be >= Low prices"
        ...
        if max_return > 0.20 or min_return < -0.20:
            print(f"  Warning: {symbol} has suspicious price movement "
...
def test_data_alignment():
    ...
    for symbol, dates in etf_dates.items():
        ...
        diff_pct = abs(dates['trading_days'] - avg_trading_days) / avg_trading_days * 100
