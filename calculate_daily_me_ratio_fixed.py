...
def calculate_daily_me_ratios(initial_value=100000):
    ...
    if missing_columns:
        print(f"ERROR: Missing required columns in trade_history.csv: {missing_columns}")
    ...
    for _, trade in trades_df.iterrows():
        print(f"  {trade['symbol']}: {trade['type']} {trade['shares']} shares, "
    ...
    for current_date in date_range:
        ...
        for _, trade in new_entries.iterrows():
            ...
            # Add to position
            shares_to_add = trade['shares'] if trade['type'] == 'long' else -trade['shares']
            position_tracker[symbol]['shares'] += shares_to_add
            position_tracker[symbol]['entry_prices'].append(trade['entry_price'])
            position_tracker[symbol]['total_cost'] += abs(shares_to_add) * trade['entry_price']
            
            print(f"  {current_date_str}: Opening {trade['type']} position in {symbol}: {abs(shares_to_add)} shares @ ${trade['entry_price']:.2f}")
        ...
        for _, trade in exits.iterrows():
            ...
            if symbol in position_tracker:
                shares_to_remove = trade['shares'] if trade['type'] == 'long' else -trade['shares']
                position_tracker[symbol]['shares'] -= shares_to_remove
                
                print(f"  {current_date_str}: Closing {trade['type']} position in {symbol}: {abs(shares_to_remove)} shares @ ${trade['exit_price']:.2f}, P&L: ${trade['profit']:.2f}")
        ...
        if len(new_entries) > 0 or len(exits) > 0 or current_date.day % 5 == 0:
            print(f"  {current_date_str}: Long=${total_long_value:,.0f}, Short=${total_short_value:,.0f}, "
    ...
    print(position_change_dates[['Date', 'ME_Ratio', 'Long_Positions', 'Short_Positions', 'Portfolio_Equity', 'Total_Position_Value']].to_string(index=False))
    
    # Save summary for quick access
    try:
        summary = {
            'average_me_ratio': float(me_df['ME_Ratio'].mean()) if len(me_df) > 0 else 0.0,
            ...
            'average_positions': float(me_df['Total_Positions'].mean()) if len(me_df) > 0 else 0.0,
            'calculation_date': datetime.now().isoformat(),
            'total_days': len(me_df),
            'total_trades': len(trades_df),
            'initial_value': initial_value,
            'final_equity': float(me_df['Portfolio_Equity'].iloc[-1]) if len(me_df) > 0 else initial_value,
            'total_profit': float(me_df['Cumulative_Profit'].iloc[-1]) if len(me_df) > 0 else 0.0
        }
    ...
    if len(me_df) > 0:
        try:
            ...
            # Position values
            ax2.plot(me_df['Date'], me_df['Long_Value'], label='Long Value', color='green')
            ax2.plot(me_df['Date'], me_df['Short_Value'], label='Short Value', color='red')
            ax2.plot(me_df['Date'], me_df['Total_Position_Value'], label='Total Position Value', 
            ...
            # Portfolio equity
            ax3.plot(me_df['Date'], me_df['Portfolio_Equity'], label='Portfolio Equity', color='purple')
            ax3.axhline(y=initial_value, color='gray', linestyle=':', label=f'Initial (${initial_value:,})')
            ax3.set_xlabel('Date')
