...
HISTORICAL_DATA_PATH = "/path/to/historical/data"  # <-- Define this before using it
...
class DailyMERatioCalculator:
    def __init__(self, initial_portfolio_value: float = 1000000) -> None:
        ...
        self.daily_me_history: List[float] = []
    ...
    def perform_me_rebalancing(self, action: str) -> bool:
        ...
        try:
            ...
            if action == "scale_up":
                ...
                target_me = (
                    self.me_target_min + self.me_target_max
                ) / 2  # 65% instead of 50%
...
def load_polygon_data(
    ...
    """Load Polygon data for given symbols"""
...
def run_ngs_automated_reporting(comparison=None) -> None:
...
def load_data(path, data_format) -> dict:
    if data_format == "json":
