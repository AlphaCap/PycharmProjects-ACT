...
class DailyMERatioCalculator:
    ...
    def update_position(self, symbol: str, shares: int, entry_price: float, 
                       current_price: float, trade_type: str = 'long') -> None:
        if shares == 0:
    ...
    def update_all_positions_with_current_prices(self, current_prices: Dict[str, float]) -> None:
    ...
    def calculate_daily_me_ratio(self, date: Optional[str] = None, current_prices: Optional[Dict[str, float]] = None) -> Dict[str, Any]:
        if date is None:
...
class NGSStrategy:
    ...
    def __init__(self, account_size: float = 1000000, data_dir: str = 'data') -> None:
        ...
        self.cutoff_date: datetime = datetime.now() - timedelta(days=self.retention_days)

        self.me_calculator: DailyMERatioCalculator = DailyMERatioCalculator(initial_portfolio_value=account_size)
...
# ... (Continue with further methods, fully type-annotated, in the next section) ...
    ...
    def update_me_calculator_with_current_prices(self, current_prices: Dict[str, float]) -> None:
    ...
    def enable_sector_rebalancing(self, custom_targets: Optional[Dict[str, float]] = None) -> None:
    ...
    def get_rebalance_candidates(self, positions_df: pd.DataFrame) -> Dict[str, List[str]]:
        ...
        for sector in sell_sectors:
            sector_positions = [symbol for symbol in current_positions if get_symbol_sector(symbol) == sector]
            sell_candidates.extend(self.filter_sell_candidates(sector_positions, positions_df))
        return {
            "buy": buy_candidates,
            "sell": sell_candidates,
            "hold": [symbol for symbol in current_positions if symbol not in buy_candidates + sell_candidates]
        }
    ...
    def calculate_historical_me_ratio(self, current_prices: Optional[Dict[str, float]] = None) -> float:
        return float(self.me_calculator.calculate_daily_me_ratio(current_prices=current_prices)['ME_Ratio'])

    def record_historical_me_ratio(self, date_str: str, trade_occurred: bool = False, current_prices: Optional[Dict[str, float]] = None) -> None:
    ...
    def _calculate_linear_regression(self, df: pd.DataFrame) -> None:
        def linear_reg(series: pd.Series, period: int, shift: int) -> Tuple[float, float, float, float]:
    ...
    def _check_short_signals(self, df: pd.DataFrame, i: int) -> None:
        if (df['Open'].iloc[i] > df['Close'].iloc[i-1] and
            ...
            adjusted_position_size = self.get_short_position_size(self.inputs['PositionSize'])
            df.loc[df.index[i], 'Shares'] = int(round(adjusted_position_size / df['Close'].iloc[i]))
    def _check_long_exits(self, df: pd.DataFrame, i: int, position: Dict[str, Any]) -> None:
    ...
    def _check_short_exits(self, df: pd.DataFrame, i: int, position: Dict[str, Any]) -> None:
        possible_exits: List[Tuple[str, str, int, Optional[int]]] = []
    ...
    def _process_exit(self, df: pd.DataFrame, i: int, symbol: str, position: Dict[str, Any]) -> None:
    ...
    def _process_entry(self, df: pd.DataFrame, i: int, symbol: str, position: Dict[str, Any]) -> None:
    ...
    def get_current_positions(self) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
...
def load_polygon_data(symbols: List[str], start_date: Optional[str] = None, end_date: Optional[str] = None) -> Dict[str, pd.DataFrame]:
    if end_date is None:
        end_date = datetime.now().strftime('%Y-%m-%d')
    if start_date is None:
        start_date = (datetime.now() - timedelta(days=RETENTION_DAYS + 30)).strftime('%Y-%m-%d')
