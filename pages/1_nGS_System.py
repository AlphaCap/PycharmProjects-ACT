...
def main():
    logger.info(f"Starting historical data scheduler - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    try:
        ...
        logger.info(f"Completed daily data fetch for {len(daily_data) if daily_data else 0} symbols")
        ...
        logger.info(f"Completed minute data fetch for {len(minute_data) if minute_data else 0} symbols")
