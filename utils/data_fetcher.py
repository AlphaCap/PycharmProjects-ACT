...
class DataFetcher:
    ...
    def _get_sp500_symbols(self):
        ...
        return [
            ...
            "MSFT", "NVDA", "AAPL", "AMZN", "GOOG", "META", "AVGO", "TSLA", "BRK.B", "V", "JNJ", "PG", "JPM"
        ]
    ...
    def _update_or_save_data(self, symbol, new_data, data_type, retention_days):
        ...
        try:
            if os.path.exists(file_path):
                ...
                logger.info(f"Updated daily data for {symbol} with {len(new_data)} new bars")
            else:
                new_data.to_csv(file_path, index=False)
                logger.info(f"Saved new daily data for {symbol} with {len(new_data)} bars")
        except Exception as e:
